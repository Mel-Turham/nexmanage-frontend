...
const ContractForm = ({ initialDate }: ContractFormProps) => {
  ...
  useEffect(() => {
    if (initialDate) {
      ...
      // Set default end time to 5:00 PM on the selected date
      const endDate = new Date(initialDate);
      endDate.setHours(17, 0, 0, 0);

      form.setValue("heureDebut", startDate);
      form.setValue("heureFin", endDate);
    }
  }, [initialDate, form]);
  ...
  const onSubmit = async (data: ContractSchema) => {
    ...
    try {
      // In a real app, you would use the mutation
      ...
      setTimeout(() => {
        ...
        // Redirect to the contracts list or details page
