...
const mockEmployee = {
  ...
  contrats: [
    { id: "1", description: "Maintenance bureau A", date: new Date("2025-01-20"), statut: "TERMINE" },
    { id: "2", description: "Réparation équipements", date: new Date("2025-01-18"), statut: "EN_COURS" },
    { id: "3", description: "Installation matériel", date: new Date("2025-01-15"), statut: "TERMINE" },
  ],
  conges: [
    {
      id: "1",
  ...
  presences: [
    { date: new Date("2025-01-20"), heureArrivee: "08:00", heureDepart: "17:00", statut: "PRESENT" },
    { date: new Date("2025-01-19"), heureArrivee: "08:15", heureDepart: "17:00", statut: "RETARD" },
    { date: new Date("2025-01-18"), heureArrivee: "08:00", heureDepart: "17:00", statut: "PRESENT" },
  ],
}
      ...
      <div className="max-w-6xl mx-auto">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <Badge className={employee.isActif ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}>
        ...
        <Tabs defaultValue="informations" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{employee.adresse}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{employee.departement}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{formatDate(employee.dateEmbauche)}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{employee.salaire.toLocaleString()} €</p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="space-y-6">
                <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Contrats assignés</span>
                      <span className="font-semibold">{employee.contrats.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Congés pris</span>
                      <span className="font-semibold">{employee.conges.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
          ...
          <TabsContent value="contrats" className="space-y-6">
            <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader>
                <CardTitle>Contrats assignés ({employee.contrats.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {employee.contrats.map((contrat) => (
                    <div key={contrat.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div>
                        <p className="font-medium text-gray-900">{contrat.description}</p>
                        <p className="text-sm text-gray-600">{formatDate(contrat.date)}</p>
                      </div>
                      <Badge className={getStatusColor(contrat.statut)}>{contrat.statut.replace("_", " ")}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          <TabsContent value="conges" className="space-y-6">
            <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader>
                <CardTitle>Demandes de congés ({employee.conges.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {employee.conges.map((conge) => (
                    <div key={conge.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div>
                        <p className="font-medium text-gray-900">{conge.motif}</p>
                        <p className="text-sm text-gray-600">
                          Du {formatDate(conge.dateDebut)} au {formatDate(conge.dateFin)}
                        </p>
                      </div>
                      <Badge className={getStatusColor(conge.statut)}>{conge.statut}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
                  ...
                  {employee.presences.map((presence, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">{formatDate(presence.date)}</p>
                          <p className="text-sm text-gray-600">
                            {presence.heureArrivee} - {presence.heureDepart}
                          </p>
                        </div>
                      </div>
                      <Badge className={getStatusColor(presence.statut)}>{presence.statut}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
